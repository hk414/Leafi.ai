...
new Rellax(".btc-illustration", {
  ...
  const increaseCount = (countElement, finalCount, currentCount, interval) => {
      ...
      if (currentCount <= finalCount) {
          setTimeout(() => increaseCount(countElement, finalCount, currentCount, interval), interval);
      }
  };
  ...
  const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
          if (entry.isIntersecting) {
              increaseCount(countElement1, finalCountIndividualLeaners, currentCount1, interval1);
              increaseCount(countElement2, finalCountUniversitiesPartners, currentCount2, interval2);
              increaseCount(countElement3, finalCountCountries, currentCount3, interval3);
              observer.disconnect();
          }
      });
  }, options);
